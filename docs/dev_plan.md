...
### ✅ Этап 4: Frontend - Client App (Quasar PWA) - карта и просмотр сервисов (ЗАВЕРШЁН)
...
**2GIS интеграция (branch: feature/2gis-map-integration):**
- [x] Интегрировать 2GIS MapGL в MapPage
- [x] Получить и настроить API ключ 2GIS
- [x] Настроить инициализацию карты с центром на Тюмени
- [x] Загрузить сервисы из API через `/businesses/nearby`
- [x] Отобразить сервисы на карте с кастомными HTML маркерами
- [x] Цветовое кодирование маркеров:
  - [x] 🟢 Зеленый: available (0-15 мин ожидания)
  - [x] 🟡 Оранжевый: busy (15-30 мин)
  - [x] 🔴 Красный: very_busy (30+ мин)
- [x] Emoji маркеры (👍) для лучшей видимости
- [x] Клик на маркер → открытие карточки с информацией
- [x] Карточка бизнеса с детальной информацией:
  - [x] Название, тип, адрес
  - [x] Текущий статус и время ожидания
  - [x] Телефон с кнопкой "Позвонить"
  - [x] Кнопка "Записаться" (placeholder)
- [x] Фильтрация по типу сервиса (автомойка/ремонт/шиномонтаж)
- [x] Добавлены реальные бизнесы: Автомойка Тура, Автомойка Пена (Тюмень)
- [x] Точная геолокация из 2GIS URLs (извлечение lat/lon из firm ID)
- [x] Troubleshooting документация (VPN issues)

**Что не вошло в этап (перенесено в Этап 5):**
- [ ] Страница детального просмотра сервиса (отдельная страница)
- [ ] Список услуг с ценами на странице сервиса
- [ ] Кнопка "Построить маршрут" (интеграция с 2GIS routing)
- [ ] Поиск по названию

**Результат:** ✅ Клиенты видят сервисы на карте Тюмени с актуальной загруженностью в реальном времени. Работает цветовая индикация статусов, фильтрация по типам, клик на маркер открывает карточку.

**Тестовые данные:**
- Автомойка Тура: 🟡 busy (20 мин) - ул. Интернациональная, 75
- Автомойка Пена: 🔴 very_busy (45 мин) - ул. Калинина, 39
- 2 тестовых автомойки: 🟢 available

**URL приложения:** http://localhost:9002

**Дата завершения:** 30 ноября 2025

---

### Этап 5: Система бронирования и дополнительные фичи клиента (неделя 5-6)
